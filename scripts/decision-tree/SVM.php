 ...
 *
 * Run:
 *   php SVM.php <user_id> <case_num> [kernel]
 */

//////////////////////////////// CONFIG ////////////////////////////////
$SAVE_MODEL = true; // nyalakan simpan model
$MODEL_DIR_FALLBACK = getcwd() . DIRECTORY_SEPARATOR . 'svm_models'; // jika bukan proyek Laravel

///////////////////////////// HELPERS /////////////////////////////////
function envv(string $k, $d=null){ $v=getenv($k); if($v!==false&&$v!=='')return $v;
  if(isset($_ENV[$k])&&$_ENV[$k]!=='')return $_ENV[$k];
  if(isset($_SERVER[$k])&&$_SERVER[$k]!=='')return $_SERVER[$k]; return $d; }
function norm(string $s): string{ $s=strtolower($s); $s=preg_replace('/[^a-z0-9]+/i','_',$s);
  $s=preg_replace('/_+/', '_', $s); return trim($s,'_'); }
function findGoalKey(array $cols,string $wanted):?string{
  if(in_array($wanted,$cols,true))return $wanted;
  $wn=norm($wanted); $m=[]; foreach($cols as $c){ $m[norm($c)]=$c; } return $m[$wn]??null; }
function table_exists(mysqli $db,string $schema,string $table):bool{
  $schema=$db->real_escape_string($schema); $table=$db->real_escape_string($table);
...
function parseKernel(string $spec): array{
  ...
  foreach(array_slice($parts,1) as $kv){
    if(strpos($kv,'=')!==false){ [$k,$v]=explode('=',$kv,2); $k=trim($k); $v=trim($v);
...
function buildFeatureMapper(array $baseIndex,array $kcfg): array{
  ...
  if($kcfg['type']==='sgd'){
    $f=fn(array $x)=>$x; return [$f,['type'=>'sgd'],$B];
  }
  ...
  // Sigmoid: random tanh features (aproksimasi praktis)
...
/** Logging gagal + exit (jika DB tersedia) */
...
global $argv,$argc; if($argc<3){ fwrite(STDERR,"Usage: php SVM.php <user_id> <case_num> [kernel]\n"); exit(1); }
$userId=(int)$argv[1]; $caseNum=(int)$argv[2]; $kernelSpec=$argv[3]??'sgd';
...
if(count($classes)<2) log_and_exit_fail($db,$userId,"Butuh >=2 kelas untuk SVM. Ditemukan: ".implode(',',$classes));
$posLabel=$classes[0]; $negLabel=$classes[1];
...
while($row=$res->fetch_assoc()){
  ...
  foreach($row as $c=>$v){
    if(in_array($c,$skipCols,true)) continue;
    if(isset($nums[$c])){
      $min=$nums[$c]['min']; $max=$nums[$c]['max']; $f=(float)$v;
      $z=($max>$min)?($f-$min)/($max-$min):0.0; $xBase[$baseIndex["NUM::$c"]]=$z;
    } elseif(isset($cats[$c])){
      $s=trim((string)$v); if($s!==''&&isset($baseIndex["CAT::$c::$s"])) $xBase[$baseIndex["CAT::$c::$s"]]=1.0;
    }
  }
...
///////////////////////////// TRAIN (SGD) //////////////////////
$lambda=1e-4; $epochs=20; $eta0=0.1;
...
if($SAVE_MODEL){
  ...
  $model = [
    'type'=>'svm_sgd','dim'=>$dim,'weights'=>$W,'bias_index'=>$biasIndex,
    'lambda'=>$lambda,'epochs'=>$epochs,'eta0'=>$eta0,
    'goal_column'=>$goalKey,'label_map'=>['+1'=>$posLabel,'-1'=>$negLabel],
    'feature_index'=>$baseIndex,'numeric_minmax'=>$nums,
    'kernel'=>$kcfg['type'],'kernel_meta'=>$kernelMeta,
  ];
  if(@file_put_contents($modelPath, json_encode($model, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES))===false){
...
$status='success';
$output="SVM SGD {$kcfg['type']}. source={$sourceTable}; goal={$goalKey}; classes={$posLabel}|{$negLabel}; ".
